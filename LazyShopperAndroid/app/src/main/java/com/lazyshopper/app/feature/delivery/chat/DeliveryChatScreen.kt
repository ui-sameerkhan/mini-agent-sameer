...
@Composable
fun DeliveryChatScreen(
    onBack: () -> Unit,
    viewModel: DeliveryChatViewModel = hiltViewModel(),
) {
    ...
    Column(Modifier.fillMaxSize()) {
        Row(Modifier.fillMaxWidth().padding(ScreenPadding), verticalAlignment = Alignment.CenterVertically) {
            ...
            Text(state.customerName?.let { "Chat with $it" } ?: "Order chat", style = MaterialTheme.typography.titleMedium)
        }
        Divider()
...
private fun ChatBubble(msg: ChatMessage) {
    val isMine = msg.sender_role == "delivery"
