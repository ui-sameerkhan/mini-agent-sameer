...
import com.lazyshopper.app.feature.delivery.util.uriToMultipart
import kotlinx.coroutines.launch

@OptIn(ExperimentalPermissionsApi::class)
@Composable
fun DeliveryKycScreen(
    onDone: () -> Unit,
    viewModel: DeliveryKycViewModel = hiltViewModel(),
) {
    ...
    Column(
        ...
        Text("Delivery Partner KYC", style = MaterialTheme.typography.headlineSmall)
        Spacer(Modifier.height(4.dp))

        when (state.kycStatus) {
            "submitted" -> StatusBanner("Your documents are under review. We'll notify you once approved.", LsWarning)
            "rejected" -> StatusBanner("Rejected: ${state.rejectReason ?: "please review and resubmit your documents."}", LsError)
            else -> Text(
                "Upload the required documents so you can start accepting deliveries.",
        ...
        DocPicker("Driving licence", state.dlId != null, state.uploadingField == "dl") { uri ->
            viewModel.uploadDoc("dl", uriToMultipart(context, uri))
        }

        Spacer(Modifier.height(16.dp))
        LsTextField(value = state.vehicleNumber, onValueChange = viewModel::setVehicleNumber, label = "Vehicle number")
        Spacer(Modifier.height(12.dp))
        LsTextField(value = state.addressText, onValueChange = viewModel::setAddressText, label = "Address (optional)")
